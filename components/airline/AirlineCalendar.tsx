    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-gray-900">即将到来的促销</h2>
        </div>
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="font-semibold text-gray-900 text-sm">{p.airline}</div>
                    <div className="text-xs text-gray-400 mt-0.5">每月 {p.day} 号</div>
                  </div>
                  <div
                    className="text-2xl font-bold tabular-nums leading-none"
                    style={{ color: p.daysUntil === 0 ? '#ef4444' : p.color }}
                  >
                ...
                {p.stackPlatforms && p.stackPlatforms.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    <span className="text-xs text-gray-400">可叠加：</span>
                    {p.stackPlatforms.map(platform => (
                      <span key={platform} className="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-medium">
                        {platform}
                      </span>
                    ))}
                  </div>
            ...
            return (
              <div
                ...
                className={`min-h-[60px] rounded-lg p-1.5 ${
                  isToday ? 'bg-blue-600 text-white' : promos ? 'bg-gray-50' : ''
                }`}
              >
                <div className={`text-xs font-semibold mb-1 ${isToday ? 'text-white' : 'text-gray-700'}`}>
                  {day}
                </div>
                {promos && (
      ...
      {/* Airline details */}
      <div>
        <h2 className="font-semibold text-gray-900 mb-4">各航司促销详情</h2>
        <div className="space-y-3">
          {AIRLINE_PROMOTIONS.map(p => (
            <div key={p.code} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-sm transition-shadow">
                ...
                <div className="flex-1 p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 hover:border-blue-300 text-gray-600 hover:text-blue-600 transition-colors"
                    >
                      官网 <ExternalLink className="w-3 h-3" />
                    </a>
                  </div>
                  <p className="mt-2.5 text-sm text-gray-600 leading-relaxed">{p.description}</p>
                  {p.stackPlatforms && p.stackPlatforms.length > 0 && (
                    <div className="flex items-center gap-2 mt-2.5">
                      <span className="text-xs text-gray-400">可叠加平台：</span>
                      {p.stackPlatforms.map(platform => (
                        <span key={platform} className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium">
                          {platform}
                        </span>
                      ))}
                    </div>
